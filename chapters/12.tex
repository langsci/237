...
What I have said so far may suggest that this is a paper about gender in personal pronouns such as \ili{English} \textit{she} (see, e.g., \citealt{Audring2009a}), but the search space is much broader. In many languages the functional equivalent to \textit{she} and \textit{her} in \ili{English} is an affix on verbs and/or adpositions as in (\ref{ex:BW:2}) from \ili{Garifuna}.
...
\ea\label{ex:BW:5}
\ili{Kiribati} (\ili{Austronesian}, \ili{Micronesian}; Matth.~15:27): grammatical anaphor\\
...
\begin{enumerate}[label=(\roman*)]
  ...
  \ea\label{ex:BW:12}
  Two of 76 verses of the trigger domain (given in the \ili{English} Lexham translation)\\
...
\ili{Zapotec, Miahuatlan} (zam)	&	\{[xa\textquotesingle]1 \}	&	\{3.310\}	\\
\lspbottomrule
\end{tabularx}
...
\item
Gender marking is restricted to a limited part of the S and P domain and the markers do not have high cue validity: \ili{Chechen}, \ili{Hindi}, \ili{Gujarati}, and Eastern Panjabi.\il{Panjabi, Eastern} These are languages with feminine genders, but the anaphoric function in those languages is marginal or non-existing. In \ili{Avar} only gender on free pronouns is detected.
\item
The marker is partly zero as opposed to a non-zero masculine marker: This holds for the \ili{Arawakan} languages \ili{Ashéninka Pajonal}, \ili{Asháninka}, \ili{Caquinte}, Pichis Ashéninka\il{Ashéninka, Pichis} and \ili{Nomatsiguenga}. The algorithm as implemented here is simply not smart enough for recognizing zero as the marker of the feminine gender gram. The recognition of zero morphemes requests some understanding of systems or at least oppositions.
\item
...
Using a larger search domain would be helpful for a few languages. With a search space of 293 verses mainly based on \ili{English} \textit{she/her} markers are extracted even for \ili{Ashéninka Pajonal} >\#ok, >\#op<, \ili{Asháninka} >\#o<, \ili{Caquinte} >\#o<, and more markers in other languages, such as \ili{Avar}, >й<, \ili{Tachelhit} >\#t<, \ili{Tamasheq} >\#tĕ<, >\#tă<, \ili{Maltese} >et<, \ili{Machiguenga} >\#os< are extracted. (Note also that \ili{Kabyle} >\#te< is only extracted with the lower threshold.) However, using a larger search domain comes at the cost of more wrong forms not filtered and nine languages with non-mature feminine gender markers and \ili{Yagua} not extracted. I have not managed to extract any forms in \ili{Hindi}, \ili{Gujarati}, Eastern Panjabi,\il{Panjabi, Eastern} \ili{Chechen}, and \ili{Iraqw}, however the extraction is designed.
...
\ea\label{ex:BW:14}
\ili{Au} (Torricelli; Mk.~3:31, Mk.~9:21, Matth.~14:11)\\
\begin{xlist}
\ex
...
All examples in \tabref{tab:BW:5} have in common that anaphoric gender marking is pronominal (whether free or bound) and has variable formal expression across grammatical relations as opposed to the invariant anaphoric gender markers of nominal origin or supposedly nominal origin discussed in \sectref{sec:BW:5}. However, not all invariant anaphoric gender markers can be proven to have nominal origin. There are, for instance, two \ili{Tupian} languages in the sample with invariant markers. \ili{Kayabi} \textit{ẽẽ} \textsc{f} (male speaker) \textsc{m}, \textit{kyna} \textsc{f} (female speaker), \textsc{m} \textit{'ga} (male speaker), and \textit{kĩã} \textsc{m} (female speaker), distinguishing both speaker and referent gender. These markers also follow person names and animate nouns in anaphoric use. \ili{Tenharim} has \textit{hẽa} \textsc{f} and \textit{'ga} \textsc{m} (singular and plural), which also occur as suffixes on referring person names and animate nouns. Like in other \ili{Tupian} languages the pronominal prefixes on nouns and verbs do not distinguish gender in \ili{Kayabi} (\citealt[27]{Dobson2005}) and \ili{Tenharim} (\citealt[17]{Betts1981}). The lack of gender markers in most \ili{Tupian} languages might suggest that anaphoric gender in \ili{Kayabi} and \ili{Tenharim} are innovations.

However, invariant marking does not always testify to recent origin of gender. \ili{Malayalam} (\ili{Dravidian}) has the constant pronominal stem \textit{avaḷ(-)} \textsc{3sg.f} and no bound pronouns. But Old \ili{Malayalam} still had subject indexes on the verb (\textit{-ǟḷ} \textsc{3sg.f}) (\citealt[120]{Andronov1996}). Anaphoric gender marking was thus not invariant in Old \ili{Malayalam}. While all \ili{Indo-European} languages and all Creole languages with anaphoric gender in the sample have variant anaphoric gender marking, the artificial language \ili{Esperanto} has invariant marking with the constant markers \textit{sxi(-)} \textsc{f} and \textit{li(-)} \textsc{m}.

Anaphoric gender can occasionally have quite unexpected sources. In \ili{Yagua}, women who have borne children are referred to by dual forms (\citealt[42]{Payne1985}) \textendash{} \textsc{3du} \textit{naada-} (often realized as \textit{naan-}), \textit{naadá}, \textsc{2du} \textit{sáána-}, \textit{saadá}. Men, however, are referred to with singular bound pronouns: \textsc{3sg} \textit{sa-} [I], \textit{-níí} [II], \textsc{2sg} \textit{jiy-} [I], \textit{jíy} [II].%
\footnote{%
Set II forms are used for direct objects and some intransitive subjects.
} %
In the N.T.\ dual forms are used as a default for adult women for whom it is not specified in the text whether they have given birth to children. Even if this is lack of gender from the point of view of the system \textendash{} and Payne (\citeyear[42]{Payne1985}) says explicitly that \ili{Yagua} lacks gender \textendash{} this is an anaphoric gender marking opposition from the point of view of language use. Anaphoric gender in \ili{Yagua} hijacks another highly grammaticalized category, number. This is why the markers are mature even if they are presumably young as gender markers. \ili{Yagua} is thus an example of a very specific origin of an anaphoric gender opposition which has a mature marker from the very beginning. However, since the origin of gender is often associated with case or number (\citealt{Waelchlithisyeara}), the example of \ili{Yagua} is perhaps less parochial than it seems at first glance.
...
\footnote{%
Since many languages also have third person singular forms not distinguishing gender they are not usually captured in Siewierska’s (\citeyear{Siewierska2005}) typology (except \ili{Japanese} where the third person singular pronoun is zero anaphor).
}%

Some of the forms to be discussed in this section figure prominently in the literature on classifiers, especially \spterm{noun classifiers}. This is no surprise since anaphoric use is a well-recognized function of noun classifiers in some languages. According to \citet[87]{Aikhenvald2000} “noun classifiers are typically used with anaphoric function”. Aikhenvald discusses especially \ili{Mayan} languages of the \ili{Kanjobalan} branch (\ili{Jacaltec} and \ili{Akateko}) and some Australian languages (notably Yidiny).\il{Yidiñ} It is thus not unexpected that some noun classifier languages are found to exhibit anaphoric gender which does not presuppose agreement as definitional property.
...
The same \ili{Jacaltec} form \textit{ix} ‘woman’ is used all the way from the nominal low activation domain up to the top pronominal domain. \textit{Naj} ‘he (non-respected, non-kin)’ is a reduced nominal anaphor (\textit{winaj} ‘man’). \textit{Ix} ‘woman; she (non-respected, non-kin)’ and \textit{naj} ‘he’ belong to the set of noun classifiers and are notably used with thematically salient NPs in referential anaphoric function (\citealt[267]{Craig1986}; \citealt[323]{Aikhenvald2000}). There are no free third person pronominal forms except classifiers. Example (\ref{ex:BW:22}) illustrates the non-respect feminine classifier \textit{ix} ‘woman’ in non-reflexive possessor and subject function and the non-respect masculine classifier \textit{naj} ‘man’ as a noun classifier in the anaphoric NP with a person name:
...
\begin{tabular}{llll}
  ...
  \textsc{act}	&	active	&	Poss1	&	inalienable or non-reflexive 	\\
  \textsc{agr}	&	agreement	&		&	possessor	\\
  \textsc{ana}	&	anaphoric	&	Poss2	&	alienable or reflexive  	\\
  \textsc{cl}	&	classifier 	&		&	possessor	\\
  \textsc{comp}	&	complementizer	&	\textsc{pot}	&	potential aspect	\\
  \textsc{compl}	&	completive aspect	&	\textsc{pp}	&	preposition	\\
  \textsc{cont}	&	continuative aspect	&	\textsc{pro}	&	pronominal	\\
  \textsc{connect}	&	connective	&	\textsc{r}	&	recipient/indirect object	\\
  \textsc{detrans}	&	detransitive	&	\textsc{resp}	&	respect	\\
  \textsc{dir}	&	directional	&	\textsc{rposs}	&	reflexive possessive	\\
  \textsc{emph}	&	emphatic 	&	\textsc{s}/S	&	intransitive subject	\\
  \textsc{incompl}	&	incompletive aspect	&	\textsc{seq}	&	sequential	\\
  \textsc{io}	&	indirect object 	&	\textsc{spec}	&	specific noun	\\
  \textsc{med}	&	medial	&	\textsc{ss}	&	same subject	\\
  \textsc{neg}	&	negation	&	V	&	verb/vowel	\\
