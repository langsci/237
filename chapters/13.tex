...
As an example, compare the numeral classifier system in \ili{Pnar} (\ili{Khasian}; \ili{Austro-Asiatic}) with that of \ili{Thai} (Kam-Tai; Tai-Kadai). \ili{Pnar} has three general classifiers used when enumerating count nouns: \textit{ŋut} for classifying people (\ref{ex:Sinne:2}a), \textit{tl̩li} for classifying non-humans (\ref{ex:Sinne:2}b), and \textit{ta} for classifying weeks (\ref{ex:Sinne:2}c) (\citealt[124--125, 361--362]{Ring2015}).%
\footnote{Note that \ili{Pnar} has gender as well, while \ili{Thai} does not (see Appendix 2).
} %

\ea
\label{ex:Sinne:2}
\langinfo{Pnar}{\ili{Khasian}; Austro-Asiatic}{\citealt[362]{Ring2015}}\\
\begin{xlist}
\ex
...
A language may also express gender-like distinctions on just the noun but not on any other constituent. For instance, in \ili{Petalcingo Tzeltal} (\ili{Mayan}) some nouns may be marked with different noun prefixes, \textit{x}{}- and \textit{j}{}- which appear in complementary distribution and if used for person's names, \textit{x}{}- is used for women's names (\ref{ex:Sinne:4}a) and \textit{j}{}- for men's names (\ref{ex:Sinne:4}b) \citep[20]{Shklovsky2005}.%
\footnote{The marker \textit{{}-e} at the end of many noun phrases in \ili{Petalcingo Tzeltal} is a determiner enclitic \citep[127]{Shklovsky2012} that apparently participates in marking the definiteness of the noun phrase. Glossing (e.g., of the \textit{x}{}- and \textit{j}{}- prefixes) follows the sources. Note that in the source the hat (\^{}) symbol marks the preceding consonant as an ejective.} %
Because there is no agreement marking on syntactic constituents reflecting the different noun types, this pattern in \ili{Petalcingo Tzeltal} and similar instances in other languages (whether the markers are affixes, clitics, or isolating formatives) were not counted as examples of grammatical gender and were left outside of this research.
...
\ea\label{ex:Sinne:6}
\langinfo{Pnar}{\ili{Khasian}; Austro-Asiatic}{\citealt[108]{Ring2015}}\\
...
Second, in some languages the set of classifiers is very limited. \ili{Marathi} (\ili{Indo-European}), for instance, has one numeral classifier \textit{jaṇ}, which is used with nouns denoting persons. A similar system occurs in some \ili{Hindi} dialects and in \ili{Nepali} (\ili{Indic}; \ili{Indo-European}; \citealt[11--12]{Emeneau1956}). Since these languages have only one numeral classifier, they were not analyzed as having a numeral classifier system. In this I follow, for instance, \citet{Nichols1992} and the \textit{Autotyp} database \citep{Bickel2017} for not analyzing languages with one numeral classifier as having a numeral classifier system.
...
\footnote{For instance, all or almost all languages in \ili{Quechuan}, \ili{Oto-Manguean}, \ili{Uto-Aztecan}, and \ili{Trans-New Guinea} language families had neither gender nor numeral classifiers, whereas some languages in the \ili{Arawakan}, \ili{Tucanoan}, and West \ili{Papuan} families had both gender and numeral classifiers (e.g.\ \ili{Palikur} in (\ref{ex:Sinne:1})).}
...
While multiple systems of noun classification are possible, they are rare (see \sectref{sec:Sinne:4.1}). One reason for languages to develop more than one system of noun classification is language contact. For instance, \ili{Santali} (Munda; \ili{Austro-Asiatic}) has two gender systems as well as numeral classifiers. One gender system is native to \ili{Santali} and it distinguishes animate from inanimate, while the other system is borrowed from \ili{Indo-Aryan} and it distinguishes male from non-male \citep[39]{Ghosh2008}. In (11), the noun \textit{Kali-idol} triggers object gender agreement on the verb, which is marked by the third person object clitic -\textit{e} that is reserved for animate beings, but it also requires the use of the a numeral classifier -\textit{taŋ}.
...
Language contact is also one reason for why multiple systems of noun classification get reduced. For instance, Retuara (\ili{Tucanoan}) has lost its classifier system and retained only a gender system because of language contact with \ili{Yucuna} (\ili{Arawakan}; see \citealt[386]{Aikhenvald2000} and references).
